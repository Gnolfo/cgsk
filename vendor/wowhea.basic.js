...
$WH.g_setTooltipLevel = function(f, a, p) {
    ...
    if (q) {
        if (!f.match(/<!--pts\d:\d:\d+(?:\.\d+)?:\d+-->/g) && !(q < 0)) {
            var c = parseInt(t[0]);
            if ($WH.isset("g_items") && g_items[c] && g_items[c].jsonequip) {
                var o = $WH.g_applyStatModifications(g_items[c].jsonequip, 0, 0, null, null, a);
                $WH.updateItemStringLink.call(this);
                f = f.replace(/(<!--ilvl-->)\d+/, function(u, j) {
                    return j + o.level
                });
                f = f.replace(/(<!--asc(\d+)-->)([^<]+)/, function(v, j, u) {
                    if (a < 40 && (u == 3 || u == 4)) {
                        return j + g_itemset_types[u - 1]
                    } else {
                        return v
                    }
                });
                f = f.replace(/(<!--dmg-->)\d+(\D+)\d+/, function(v, j, u) {
                    return j + o.dmgmin1 + u + o.dmgmax1
                });
                f = f.replace(/(<!--dps-->\D*?)(\d+\.\d+)/, function(u, j) {
                    return j + (o.dps ? o.dps.toFixed(2) : "0")
                });
                f = f.replace(/(<!--amr-->)\d+/, function(u, j) {
                    return j + o.armor
                });
                f = f.replace(/<span><!--stat(\d+)-->[-+]\d+(\D*?)<\/span>(<!--e-->)?(<!--ps-->)?(<br ?\/?>)?/gi, function(y, u, j, A, B, v) {
                    var w, z = (o[$WH.g_statToJson[u]] ? o[$WH.g_statToJson[u]] : 0);
                    if (z) {
                        z = (z > 0 ? "+" : "-") + z;
                        w = "";
                        v = (v ? "<br />" : "")
                    ...
                    return "<span" + w + "><!--stat" + u + "-->" + z + j + "</span>" + (A || "") + (B || "") + v
                });
                f = f.replace(/<span class="q2">((?:<!--stat\d+-->)?[-+])<!--rtg(\d+)-->\d+(.*?)<\/span>(<br \/>)?/gi, function(y, j, u, A, v) {
                    var w, z = (o[$WH.g_statToJson[u]] ? o[$WH.g_statToJson[u]] : 0);
                    if (z) {
                        w = "";
                        v = (v ? "<br />" : "")
                    ...
                    return '<span class="q2"' + w + ">" + j + "<!--rtg" + u + "-->" + z + A + "</span>" + v
                })
            }
        } else {
            if (q > 0) {
    ...
    if (b[h]) {
        ...
        $("#open-links-button").click(function() {
            ...
            Links.show({
                ...
                linkId: "item:" + h + ":0:0:0:0:0:0:0:" + a + ":" + f,
...
$WH.Tooltip = {
    ...
    move: function(p, o, e, q, d, b) {
        ...
        for (var h = 0, k = $WH.Tooltip.moveTests.length; h < k; ++h) {
            ...
            if ($WH.isset("Ads") && !Ads.site.intersect(m)) {
                f = true;
                break
            } else {
                if (!$WH.isset("Ads")) {
